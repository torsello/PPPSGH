...
public class DefaultEstadiaABM extends ABMGenerico<Integer, Estadia> implements EstadiaABM {
	...
	public void guardar(Estadia entidad) {
		...
		if(entidad.getDiaCheckIn().before(new Date())){
			entidad.setDiaCheckIn(new Date());
		}
		
		if((entidad.getDiaCheckIn().compareTo(entidad.getDiaCheckOut())==0)){
			entidad.setDiaCheckOut(new Date(entidad.getDiaCheckOut().getTime()+24*60*60*1000));
		}else if((entidad.getDiaCheckIn().compareTo(entidad.getDiaCheckOut())>0)){
			entidad.setDiaCheckOut(new Date(entidad.getDiaCheckIn().getTime()+24*60*60*1000));
		}
